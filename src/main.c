...
static volatile char *send_ptr = 0;
...
void init_USART()
{
	...
	// Interruptions
	USART2.CR1 = (USART2.CR1 & 0xFFFFFF9F) | 0x00000060;
...
int _async_puts(const char *s)
{
	...
	// Si s est NULL, on regarde si une chaine est en cours d'envoi
	if (s == NULL)
	{
		if (sending == 0)
		{
	...
	{
		// Si s n'est pas NULL, on regarde si une chaine est en cours d'envoi
		if (sending == 0)
		{
			sending = 1;
			// On envoie le premier caractère
			_putc(*s);
